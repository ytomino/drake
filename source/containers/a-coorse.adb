...
package body Ada.Containers.Ordered_Sets is
   ...
   is
      ...
      function Compare (Right : not null Binary_Trees.Node_Access)
         return Integer is
      begin
         return Compare (Item, Downcast (Right).Element);
      end Compare;
   begin
...
--  diff
   ...
   begin
      if Before = null or else New_Item < Before.Element then
         Unique (Container, True);
         Position := new Node'(
            Super => <>,
            Element => New_Item);
...
--  diff
   ...
   function "<=" (Left, Right : Cursor) return Boolean is
   begin
      return Left /= null and then not (Right.Element < Left.Element);
   end "<=";

   function ">=" (Left, Right : Cursor) return Boolean is
   begin
      return Left /= null and then not (Left.Element < Right.Element);
   end ">=";
   ...
   package body Generic_Keys is
      ...
      is
         ...
         begin
            return Compare (
               Key,
               Generic_Keys.Key (Downcast (Right).Element));
         end Compare;
      begin
         return Downcast (Binary_Trees.Find (
            Data.Root,
