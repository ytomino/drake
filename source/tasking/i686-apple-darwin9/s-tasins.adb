...
package body System.Tasking.Inside is
   ...
   function Master_Of_Parent (Level : Master_Level) return Master_Access is
      Actual_Level : Master_Level := Level; -- [gcc 4.5] 0 may be coming
      Parent : Task_Id := Current_Task_Id;
      Result : Master_Access;
   begin
      if Actual_Level = 0 then
         pragma Assert (Parent.Kind = Main);
         Actual_Level := Library_Task_Level;
      else
         while Parent.Master_Level >= Actual_Level loop
